...
export function Team() {
    const [teamMembers, setTeamMembers] = useState<Pengurus[]>([])
    ...
    const fetchTeam = async () => {
        try {
            ...
            if (error) {
                ...
                setTeamMembers(data as Pengurus[])
            }
        } catch (error) {
    ...
    if (loading) {
        return <div className="py-32 bg-[#0a0a0a] min-h-[500px] flex items-center justify-center text-white">Loading Team...</div>;
    }

    if (teamMembers.length === 0) {
        return (
            <div className="py-32 bg-[#0a0a0a] text-center text-gray-500">
                <p>Data Pengurus Tidak Ditemukan (0 items)</p>
                <p className="text-xs mt-2">Cek Console (F12) untuk detail error.</p>
            </div>
        )
    }
        ...
        <section id="pengurus" className="py-32 bg-[#0a0a0a] relative">
            <div className="max-w-5xl mx-auto px-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                    ...
                    {teamMembers[0] && (
                        <div
                            className="group relative col-span-2 row-span-2 aspect-square md:aspect-auto bg-[#0f0f0f] border border-[#1a1a1a] p-6 md:p-8 flex flex-col justify-between transition-all duration-500 hover:border-[#3d3d3d] hover:bg-[#111111]"
                        >
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    {teamMembers[0].instagram_url && (
                                        <a href={teamMembers[0].instagram_url} target="_blank" rel="noreferrer" className="w-8 h-8 flex items-center justify-center border border-[#2d2d2d] rounded-full text-[#4d4d4d] transition-all duration-300 hover:bg-white hover:text-[#0a0a0a] hover:border-white">
                                            <IconInstagram />
                                        </a>
                                    )}
                                    {teamMembers[0].email && (
                                        <a href={`mailto:${teamMembers[0].email}`} className="w-8 h-8 flex items-center justify-center border border-[#2d2d2d] rounded-full text-[#4d4d4d] transition-all duration-300 hover:bg-white hover:text-[#0a0a0a] hover:border-white">
                                            <IconMail />
                                        </a>
                                    )}
                                </div>
                    ...
                    {teamMembers.slice(1, 3).map((member, index) => (
                        <div
                            key={member.id}
                            className="group relative col-span-1 aspect-square bg-[#0f0f0f] border border-[#1a1a1a] p-4 md:p-5 flex flex-col justify-between transition-all duration-500 hover:border-[#3d3d3d] hover:bg-[#111111]"
                        >
                    ...
                    {teamMembers.slice(3).map((member) => (
                        <div
                            key={member.id}
                            className="group relative col-span-1 aspect-[4/3] md:aspect-square bg-[#0f0f0f] border border-[#1a1a1a] p-4 flex flex-col justify-between transition-all duration-500 hover:border-[#3d3d3d] hover:bg-[#111111]"
                        >
                    ...
                    {/* Join Card */}
                    <div
                        className="group relative col-span-1 aspect-[4/3] md:aspect-square bg-[#0f0f0f] border border-dashed border-[#2d2d2d] p-4 flex flex-col items-center justify-center text-center transition-all duration-500 hover:border-white cursor-pointer"
                    >
